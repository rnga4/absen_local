...
- Animasi Transisi & Navigasi: Animasi halaman halus saat berpindah menu (`pageEnter` blur-fade scale-in & `pageExit` slide-fade out via handler `assets/toast.js` & CSS `@view-transition` / keyframes di `assets/style.css`), plus micro-interactions pada menu drawer (`translateX(4px)` & active scale compression). **Catatan:** rule `.container, main` memakai animasi `pageEnter` TANPA `will-change`/`forwards` — keduanya dihapus karena memicu bug kompositing GPU di Android Chrome (layer permanen → `overflow:hidden` pada avatar gagal memotong foto full-res).
- Favicon & Tab Bar: semua halaman memakai `app/assets/favicon.svg` (clipboard + centang, warna `#3859A8` mendekati `--primary`) via `<link rel="icon" type="image/svg+xml" href="assets/favicon.svg">` plus `<meta name="theme-color" content="#3859A8">` di setiap `<head>` (6 halaman: `public`, `index`, `employee`, `login`, `profile`, `weekly`).
...
- **`employee.php`** — dashboard karyawan personal (role `employee`): data absen "hari ini" (in/out/status/telat), avatar & foto profil, riwayat collapsible (lazy-load) via `api_history.php`. Guard: non-employee → redirect `index.php`; tanpa `emp_code` → `logout.php`. Layout `.container { max-width: 720px; }`.
  ...
  - **Riwayat lazy-load & infinite scroll**: `#historyBody` adalah scroll container sendiri (`max-height: 60vh; overflow-y: auto; overscroll-behavior: contain`) — scroll ke bawah pada daftar memuat batch berikutnya (30 hari) via listener `scroll` di `bodyEl`, pola sama persis dgn modal di `index.php`/`weekly.php`. Tanpa `overflow-y: auto`, event scroll tidak pernah terpicu (yang scroll = halaman).
...
- **Responsive image & avatar clipping** (fix Android Chrome): reset global `img { max-width: 100%; height: auto; }` di `assets/style.css` + semua rule img avatar diberi `max-width/max-height: 100%` (`.emp-thumb img`, `.profile-avatar img`, `.pub-avatar-btn img`, `.pub-profile-avatar img`, avatar modal di `index.php:590`). Karena `photo.php` menyajikan ukuran asli (tanpa GD/resize), cap geometris ini memastikan foto tidak pernah menembus lingkarannya walau `overflow: hidden` gagal di layar GPU Android (dipicu layer permanen dari animasi — lihat bagian Arsitektur).
